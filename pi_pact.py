...
class Scanner(object):
    ...
    def process_scans(self, scans, timestamps):
        ...
        for bucket_id in scans.keys():
            cnt = 0
            for (scan, timestamp) in zip_longest(scans[bucket_id], timestamps[bucket_id]):
                advertisement = pd.Series(np.zeros(10), index = ['RSSI','ax', 'ay', 'az','gx', 'gy', 'gz', 'mx', 'my', 'mz'])
